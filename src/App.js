...
import { ThemeProvider, createTheme, CssBaseline, Drawer, List, ListItem, ListItemIcon, ListItemText, Box, Container, Typography, IconButton, AppBar as MuiAppBar, Toolbar, Divider, Paper, useMediaQuery, Avatar, Tooltip, Menu, MenuItem, Fade, Badge, Button, SwipeableDrawer } from '@mui/material';
...
const App = () => {
  ...
  const [user, setUser] = useState({ name: 'Yusuf AYKIN', role: 'Admin', avatar: '/path/to/avatar.jpg' });
  ...
  const addSale = (sale) => {
    ...
    setSales((prevSales) => [...prevSales, { ...sale, id: Date.now(), profit }]);
  ...
  const addHotelTire = (hotelTire) => {
    setHotelTires((prevHotelTires) => [...prevHotelTires, { ...hotelTire, id: Date.now() }]);
  };
    ...
    setInventory((prevInventory) => {
      ...
      if (existingItem) {
        return prevInventory.map((item) =>
          item.tireId === tireId
            ? { ...item, quantity: action === 'increase' ? item.quantity + quantity : item.quantity - quantity }
            : item
        );
      } else {
  ...
  const addDelivery = (delivery) => {
    setDeliveries((prevDeliveries) => [...prevDeliveries, { ...delivery, id: Date.now() }]);
  ...
  const renderComponent = () => {
    switch (selectedComponent) {
      ...
      case 'Analytics':
        return <Analytics sales={sales} tires={tires} />;
      case 'Inventory':
        return <Inventory inventory={inventory} tires={tires} />;
      case 'Deliveries':
        return <Deliveries deliveries={deliveries} addDelivery={addDelivery} tires={tires} />;
      default:
        return <Dashboard tires={tires} sales={sales} />;
    }
  };
